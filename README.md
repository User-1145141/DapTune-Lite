...
4. 需要自动切换时，再授予蓝牙、通知等系统请求的最小权限并配置后台策略。
...
2. 打开 DapTune，在“调音”选择平直并应用；
3. 低音量下临时对一个中频段做明显衰减，用同一播放器、同一音量和同一输出 A/B；
4. 查看结果是“曲线已回读验证”还是“写入命令已接受”，再恢复目标曲线；
5. 在“配置”导入或保存命名曲线；
6. 在“自动”设置默认配置和各设备规则，再开启自动切换；
7. 如需重启后恢复，开启对应选项，并按设备系统要求允许自启动、后台运行或取消电量限制；
8. 切换输出后查看日志，确认识别到的路由、选中的配置和写入结果。
...
自动切换只在用户显式开启时运行。服务先同步当前路由，再监听路由和 Dolby 状态事件；同一服务生命周期内，
